...
const OUTPUT = path.join(
  PROJECT_ROOT,
  "gt_designer/single-mesh-evaluation/reports/blue-hat-semantic-appearance-v2-controls.json",
);
const SCENARIOS = Object.freeze([
  { id: "approved-blue-hat-v2", variant: null, expected: "pass" },
  { id: "delete-brim-role", variant: "delete-brim-role", expected: "reject" },
  { id: "flatten-panel-system", variant: "flatten-panel-system", expected: "reject" },
  { id: "delete-motif-role", variant: "delete-motif-role", expected: "reject" },
...
async function main() {
  ...
  if (!before.passed) throw new Error(`Blue Hat positive freeze failed before controls: ${JSON.stringify(before.failures)}`);
  const scenarios = [];
  for (let index = 0; index < SCENARIOS.length; index += 1) {
    const scenario = SCENARIOS[index];
    process.stdout.write(`Blue Hat semantic control: ${scenario.id}\n`);
    const automation = await runLocalSceneAutomation({
      label: `blue-hat-semantic-v2-${scenario.id}`,
      serverFlag: "--evaluation",
      path: "/single-mesh-evaluation/",
      query: `?evaluate=object&unit=blue-hat&category-baseline=stage2-v2${
        scenario.variant ? `&appearance-variant=${encodeURIComponent(scenario.variant)}` : ""
      }`,
      readyState: { state: "evaluated", objectId: "blue-hat" },
      probeExpression: `({
  ...
  const report = {
    schemaVersion: "blue-hat-semantic-appearance-controls-v2",
  ...
  if (!report.acceptance.passed) throw new Error(`Blue Hat semantic controls failed: ${JSON.stringify(report.acceptance.failures)}`);
  if (check) {
    ...
    if (frozen.schemaVersion !== report.schemaVersion || frozen.scenarios.length !== report.scenarios.length) {
      throw new Error("frozen Blue Hat semantic controls identity differs");
    }
  } else {
    await mkdir(path.dirname(OUTPUT), { recursive: true });
    await writeFile(OUTPUT, `${JSON.stringify(report, null, 2)}\n`);
  }
  process.stdout.write("Blue Hat semantic appearance controls: PASS (1 positive, 4 rejected)\n");
}
