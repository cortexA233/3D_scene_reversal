...
import {
  ...
  evaluateQualityGate,
} from "../tools/evaluation/visual-metrics.mjs";
...
test("missing silhouette is penalized with a finite frame-diagonal distance", () => {
  ...
  assert.ok(
    Math.abs(result.silhouette.symmetricEdgeMeanPixels - Math.hypot(3, 3)) <
      1e-12,
  );
});
...
test("identical appearance is perfect inside the two-pixel eroded mask", () => {
  ...
  assert.equal(result.albedo.p90DeltaE00, 0);
  assert.equal(result.albedo.maskedSsim, 1);
  assert.equal(result.litRgb.maskedSsim, 1);
  assert.equal(result.palette.centroidDeltaE00, 0);
  assert.equal(result.palette.coverageL1, 0);
  assert.equal(result.material.roughnessAbsoluteError, 0);
  assert.equal(result.material.metalnessAbsoluteError, 0);
});
...
function passingView(overrides = {}) {
  return {
    ...
    geometry: {
      bounds: {
        maxAxisRelativeError: 0.01,
        bottomAnchorErrorCanonical: 0.01,
      },
      silhouette: {
        iou: 0.97,
    ...
    appearance: {
      ...
      material: {
        roughnessAbsoluteError: 0.05,
        metalnessAbsoluteError: 0.02,
      },
    },
    ...overrides,
  };
...
test("quality gates identify the exact failed Stone Path threshold", () => {
  const views = [passingView(), passingView()].map((view, index) => ({
    ...view,
    viewId: `view-${index}`,
    geometry: {
      ...view.geometry,
      silhouette: { ...view.geometry.silhouette, iou: 0.94 },
    },
  }));
  const result = evaluateQualityGate("stone-path", aggregateVisualEvidence(views));
  assert.equal(result.passed, false);
  assert.deepEqual(result.failures, [
    {
      metric: "geometry.silhouette.meanIou",
      actual: 0.94,
      operator: ">=",
      threshold: 0.95,
    },
  ]);
  assert.equal(result.geometryGate.passed, false);
  assert.equal(result.appearanceGate.evaluated, false);
  assert.equal(result.appearanceGate.reason, "geometry-gate-failed");
});

test("appearance gates run only after geometry passes", () => {
  const view = passingView({
    appearance: {
      ...passingView().appearance,
      albedo: {
        ...passingView().appearance.albedo,
        meanDeltaE00: 20,
      },
    },
  });
  const result = evaluateQualityGate(
    "stone-path",
    aggregateVisualEvidence([view]),
  );
  assert.equal(result.geometryGate.passed, true);
  assert.equal(result.appearanceGate.evaluated, true);
  assert.equal(result.appearanceGate.passed, false);
  assert.equal(result.failures[0].metric, "appearance.meanDeltaE00");
});

test("quality gates pass complete compliant Stone Path evidence", () => {
  const result = evaluateQualityGate(
    "stone-path",
    aggregateVisualEvidence([passingView(), passingView()]),
  );
  assert.equal(result.passed, true);
  assert.deepEqual(result.failures, []);
  assert.equal(result.geometryGate.passed, true);
  assert.equal(result.appearanceGate.evaluated, true);
  assert.equal(result.appearanceGate.passed, true);
});
