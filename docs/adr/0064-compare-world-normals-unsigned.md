...
A smaller asymmetry surfaced. `run-fixed-camera-calibration.mjs --aggregate` recomputes `aggregateCameras` from the rows stored in each control partial, so a change to how rows are *combined* is free on the calibration side. The candidate's `scene-passes-v1.json` had no such mode — the browser wrote its aggregate at capture time — so the six-camera passes had to be re-captured for a change that touched no pass.

That is fixed rather than left as a note: `npm run aggregate:scene-passes` re-combines the stored rows offline. It reproduces the captured aggregate byte for byte, which is worth more than the saved minutes — it is a standing cross-check that the browser-side and Node-side `aggregateCameras` really are one implementation, of exactly the kind whose absence let two copies of a sampling rule drift apart in ADR-0055. It only re-combines: a change to what a *pass measures* still invalidates every capture and still needs the browser, and this cannot tell the difference, which is the same limit the calibration side's `--aggregate` has.
