...
- Blue Hat's hash-frozen profile-shell candidate uses 58 Object-specific Scalars, two draw calls, 704 triangles, and an analytic eight-panel/motif program. It passes the unchanged category-v1 geometry gate at mean/worst silhouette IoU `0.99608/0.99478` and all nonvisual ceilings.
- `blue-hat-semantic-category-baseline-v2` treats exact-position complex-texture DeltaE, SSIM, and palette clustering as diagnostic while hard-gating bounded brim, dark-panel, light-panel, and motif Semantic Material Role Coverage plus unchanged roughness and metalness. Brim deletion, panel flattening, motif deletion, and wrong-palette controls all reject with geometry still passing.
- The Blue Hat candidate passes Chrome and two stable full-protocol native hardware-GPU repetitions in Firefox and Safari. Its category-v1 exact-position appearance FAIL remains historical evidence; Candle is the only unresolved object before formal 8/8 certification.
- Candle's hash-frozen candidate uses the measured 20-point closed pedestal profile, a 12-direction carved stand/tray, wax pillar, and curved wick in 83 Object-specific Scalars, three draw calls, and 672 triangles. It passes unchanged category-v1 geometry at mean/worst silhouette IoU `0.99491/0.99273` and all nonvisual ceilings.
- `candle-semantic-category-baseline-v2` preserves category-v1 geometry, roughness, and metalness while making exact-position textured-stone DeltaE, SSIM, and palette clustering diagnostic. Dark/mid/light stone, wax, and wick roles are hard; flattened stone, deleted wax, deleted wick, and wrong-palette controls reject with geometry still passing.
- The Candle candidate passes Chrome and two stable full-protocol native hardware-GPU repetitions in Firefox and Safari. Its category-v1 exact-position appearance FAIL remains historical evidence; all eight Procedural Replacements now have accepted object evidence, but formal exit still requires the unified Eight-slot Lab Reference Layout and 8/8 certification.
- `single-mesh-stage-2-eight-object-certification-v1` reports `PASS under versioned category-specific baselines (8/8)`. It verifies all eight Chrome/nonvisual acceptances, every object's Firefox and Safari native hardware-GPU evidence, current hash-frozen positives, complex-appearance destructive controls, and eight matching normalized bottom-center slots.
- The formal Reference-layout Delivery is the single `/stage-1-5-scene/` route populated with all eight Procedural Replacements. It loads no Authored Reference asset, does not use isolated per-object evaluation pages as final scenes, preserves historical Stage 1 as `2/4 FAIL`, and makes no full-island claim.
