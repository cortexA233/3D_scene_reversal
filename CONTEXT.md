...
- Two Foundation limits are recorded as real blockers rather than passed checks: native Firefox and Safari GPU scene gates cannot run because neither browser is installed on this normative host, and the fixed-camera and native-appearance layers have no calibrated thresholds yet, so they are reported as not evaluated and cannot pass.
...
- Five of the six frozen evaluation cameras see only cloud and fog rather than the island: the top-down camera sits at y=4106 and the four obliques at y=3127, the cloud sprites occupy y about 500 to 2600 between them and the island, and linear fog ends at 3500 while the island is 3100 to 4100 units away. Aggregate fixed-camera numbers are therefore dominated by sky agreeing with sky, and a per-group IoU of 0 on an oblique view usually means the group is invisible from there rather than wrong. Calibrating the fixed-camera and appearance layers against those views would freeze thresholds that measure nothing, so the auxiliary camera set needs a versioned migration under ADR-0036 before that calibration is meaningful.
