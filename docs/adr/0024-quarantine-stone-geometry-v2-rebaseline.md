---
status: accepted
---

# Quarantine Stone's geometry v2 rebaseline

After the representation-validity review closed by ADR-0026,
Stone's corrected 24-direction Bounded Support-plane Polyhedron may receive one
separately versioned `stone-geometry-baseline-v2` if it still fails v1, without
changing `single-mesh-quality-baseline-v1`, the historical negative reports,
the representation, or any nonvisual ceiling. Because the candidate metrics
would then be known, this is a Quarantined Rebaseline: the corrected generator,
recipe, and evidence are commit- and hash-frozen; a complete Authored Reference-only
perturbation manifest and deterministic threshold-selection rule freeze before
calibration; and the candidate is excluded until the new limits are immutable.

Each hard limit is the least-permissive value containing the declared mild
envelope plus a predeclared repeatability allowance, and it is valid only when
all applicable destructive controls still fail. A non-separating metric is
replaced or made diagnostic rather than moved toward the candidate. The exact
frozen candidate is then evaluated without refitting in Chrome, native
Firefox, and native Safari. A calibration overlap or candidate failure ends
this restart and permits no further Stone threshold relaxation; passing all
visual and unchanged nonvisual gates reopens Stage 1.5 at Umbrella's patterned
appearance calibration.
