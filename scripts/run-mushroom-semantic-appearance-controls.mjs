...
const OUTPUT = path.join(
  PROJECT_ROOT,
  "gt_designer/single-mesh-evaluation/reports/mushroom-semantic-appearance-v2-controls.json",
);
const SCENARIOS = Object.freeze([
  { id: "approved-mushroom-v2", variant: null, expected: "pass" },
  { id: "delete-cap-role", variant: "delete-cap-role", expected: "reject" },
  { id: "delete-stem-role", variant: "delete-stem-role", expected: "reject" },
...
async function main() {
  ...
  if (!before.passed) throw new Error(`Mushroom positive freeze failed before controls: ${JSON.stringify(before.failures)}`);
  const scenarios = [];
  for (let index = 0; index < SCENARIOS.length; index += 1) {
    const scenario = SCENARIOS[index];
    const automation = await runLocalSceneAutomation({
      label: `mushroom-semantic-v2-${scenario.id}`,
      serverFlag: "--evaluation",
      path: "/single-mesh-evaluation/",
      query: `?evaluate=object&unit=mushroom&category-baseline=stage2-v3${
        scenario.variant ? `&appearance-variant=${encodeURIComponent(scenario.variant)}` : ""
      }`,
      readyState: { state: "evaluated", objectId: "mushroom" },
      probeExpression: `({
  ...
  const report = {
    schemaVersion: "mushroom-semantic-appearance-controls-v2",
  ...
  if (!report.acceptance.passed) throw new Error(`Mushroom semantic controls failed: ${JSON.stringify(report.acceptance.failures)}`);
  if (check) {
    ...
    if (frozen.schemaVersion !== report.schemaVersion || frozen.scenarios.length !== report.scenarios.length) {
      throw new Error("frozen Mushroom semantic controls identity differs");
    }
  } else {
    await mkdir(path.dirname(OUTPUT), { recursive: true });
    await writeFile(OUTPUT, `${JSON.stringify(report, null, 2)}\n`);
  }
  process.stdout.write("Mushroom semantic appearance controls: PASS (1 positive, 3 rejected)\n");
}
