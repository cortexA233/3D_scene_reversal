...
- `scene-quality-baseline-v1` is frozen from 14 scene, 7 geography, and 5 horizon reference-only controls. Every metric is calibrated only against the damage it declares it detects, every declared damage control is caught by at least one gating metric, every mild control passes, and semantic identity metrics are exact zero rather than a calibrated tolerance. 19 metrics gate and none were demoted. Its placement and surface limits are absolute world units calibrated with the 1.4 km horizon groups included, so they are generous for small entities; scale-relative placement evidence is the declared next refinement.
- The first Scene Parity Gate Stack run is `candidate-failure`: structural correspondence passes, world geometry fails on nine metrics (surface p95 11.22 against 2.35, worst entity 165.67 against 13.76, over-tolerance fraction 0.62 against 0.12, worst component delta 9 against 3.5, terrain height p95 16.26 against 5.27, shore height p95 12.49 against 2.85, land and sea agreement 0.903 against 0.931, horizon profile p95 0.0989 against 0.0165, worst azimuth 0.125 against 0.0165), and the fixed-camera and native-appearance layers cannot pass because they have no frozen thresholds yet. Coastline symmetric p95 15.75 and area error 1.97% both pass.
- The Reference-guided Fitting Loop is proven on one narrow tracer: a single `spreadScale` control per Horizon Group, fitted against the reference's own measured skyline and persisted into the Scene Recipe. Sixteen numbers are added and nothing else; the loop resets to the neutral state, measures, fits, regenerates through the real production build, and re-measures, moving `horizon.groups.silhouetteError.mean` from 0.048153 to 0.047770 while 60 unrelated entities stay byte-identical in derived seeds and CPU geometry. A fit is accepted only when it beats the unfitted result and preserves every azimuth the reference covers, so the search can never trade skyline coverage for a lower angular error.
- `scene-parity-foundation-certification-v1` records `foundation=PASS` with `candidate=RED`, which is the milestone's intended result. Repeated generation is byte-identical across 711 semantic IDs; the island builds and renders from 11 production files in an isolated root with the Authored Reference, its data, and every development tool absent, making 12 local code requests, 0 external requests, and 32,825 B gzip; and the candidate evaluation command itself exits non-zero. Ranked residuals for the reconstruction milestone are terrain elevation, native appearance, object surfaces, skyline, semantic structure, distributed cover, and fixed-camera silhouette, in that order.
- One Foundation limit remains a real blocker rather than a passed check: native Firefox and Safari GPU scene gates cannot run because neither browser is installed on this normative host. The other, the two uncalibrated gate layers, was resolved by ADR-0051. The certification now derives that list from the gate stack rather than stating it in prose, because the written sentence outlived the condition it described.
- Semantic structure is measured as a one-sided component *deficit*, not an absolute delta. Missing structure is the failure the check exists for; a procedural form that decomposes differently is not, and penalising extra parts equally pushed every generator toward a single mass. The calibrated metric and its `strip-components` control moved to the deficit accordingly.
- Vegetation canopies are reconstructed from layered curved blade geometry with each canopy merged into one semantic part. Per-group silhouette IoU rose from 0.658 to 0.729, palm surface p95 fell to 5.80 and blossom to 3.57, aggregate surface p95 fell from 11.19 to 9.87, and draw calls fell from 3,389 to 1,699. Generation time rose to 3.2 s and triangles to 678k, both carried to the runtime-budget ticket.
- Five of the six frozen evaluation cameras saw only cloud and fog rather than the island: the top-down camera stood at y=4106 and the four obliques at y=3127, the cloud band reached y=3174.86 between them and the island, and linear fog ends at 3500 while the island was 5727 units away. Aggregate fixed-camera numbers were therefore dominated by sky agreeing with sky, and a per-group IoU of 0 on an oblique view usually meant the group was invisible from there rather than wrong. ADR-0049 re-derived the five auxiliary cameras against the island subject and re-froze the set as `reference-camera-set-v2`; the margins and every threshold are unchanged. Oblique standoff is now 946 units, the island fills 4.05 to 16.21 per cent of each frame, and no group is at zero pixels. The honest metrics all got worse as a result, which is what should happen when cameras stop measuring fog.
- Whole-frame image metrics are not island evidence on the auxiliary cameras, because geography fills 84 to 86 per cent of those frames. Whole-frame silhouette IoU reads 0.994 while per-group IoU is 0.422, and whole-frame contour distance p95 measures 0 pixels on the top-down camera and 1 pixel on all four obliques whatever the island looks like. The rendered gate layers gate per-group and per-Material-Family metrics only.
- The fixed-camera and native-appearance layers are calibrated (ADR-0051) from declared scene-space damage to the reference's own scene graph, rendered through the same six frozen cameras. The bracket reuses the world-space bracket's values so that mild and severe mean one thing across the stack, and the baseline's contents advanced to revision `scene-quality-baseline-v1.1` with no geometry threshold moved. Calibration is the one thing permitted to mutate the Assembled Authored Scene, and each capture proves its own restore numerically and by re-rendering a frame to compare byte for byte with the undamaged one.
- Per-pixel world-normal error over a silhouette intersection is saturated at this scene's scale and had to be repaired before it could gate anything. A 0.15-unit translate produced a per-group p95 of 45.75 degrees, because most pixels sit on geometry one or two pixels across and a sub-pixel shift makes a pixel see a different surface rather than the same surface turned. The candidate's 71.22 degrees sat inside that mild band. Orientation is now compared only where linear depth also agrees within 2 world units, the world-geometry layer's own surface tolerance being 2.3479.
- Every mask pass wrote an identity that a per-instance colour then multiplied. three.js applies an `InstancedMesh`'s `instanceColor` to whatever material is bound, so the silhouette, semantic-group, and Material-Family passes, and the inventory's own visibility pass, all read back a tinted identity for any instanced mesh carrying one. The reference's two ground-rock populations call `setColorAt` and its three grass populations do not, so 5,100 authored instances decoded as another group: `cover`'s reference mask held 128 pixels on the authored overview where the scene draws 5,299, and every one of the 5,172 missing pixels was credited to `plazas`, whose mask was 84 per cent scatter. The depth and world-normal `ShaderMaterial`s separately ignored `instanceMatrix` and rendered every instance on its mesh's origin, and the identity lattice could not carry the fourteenth declared group. ADR-0053 corrects all three, moves the encodings into a module that is checked by analytical fixtures without a GPU, and re-derives the two rendered layers as `scene-quality-baseline-v1.2`; the worst-group intersection and worst semantic confusion both got stricter, because their previous brackets were measured through the defect.
- Distributed Scene Cover cannot be gated by a per-pixel silhouette intersection, and the frozen bracket says so without reference to any candidate. Through the corrected passes a *mild* 0.02-radian yaw of the reference against itself takes `cover`'s IoU to 0.0415 and a mild one per cent scale to 0.1344, against a worst-group threshold of 0.302569: the group is 5,100 instances one to a few pixels across, so once anything moves further than an instance's own footprint there is no intersection left. The worst-group intersection therefore ranges over groups whose members carry individual identity, exactly as the definition of the term requires, while cover stays in every mean and in the contour-distance and depth maxima whose own brackets do separate on it — 17.9 to 268.6 pixels and 5.3 to 86.5 world units.
- A cover population's rendered footprint is set by the shape of its size distribution, not by its total surface area, and the difference is not small. The authored ground rocks draw size as `min + pow(uniform, exponent) * (max - min)` with measured exponents of 2.148 and 2.317, sink 0.271 and 0.291 of each instance's own scale into the terrain, and three of the five populations are two-triangle blades with no thickness rooted at their base rather than centred lumps. Matching the measured total area with a uniform range instead put the median at 1.5 times the authored one with no instance below 0.71, so every candidate instance cleared a pixel where the reference renders most of them below one. `tools/development/measure-cover-instances.mjs` measures the ladder, the sink, and the authored form's extent and origin height, and the recipe carries ten measured numbers per population with the contract rejecting a population that omits any of them.
- The Bounded Semantic Terrain Program's frozen budget of 40 named analytic landforms cannot reach its height gates, and the boundary is one of count rather than of representation family. Running the production landform pursuit at increasing budgets against the frozen elevation evidence, 40 forms reach a full height p95 of 8.657 against a threshold of 5.26875, 80 reach 5.937 and 120 reach 4.673, so the gate is crossed at about 101 forms; shore p95 does not cross its own 2.85035 at any budget tested, flattening at 4.121 by 320 forms. Those are greedy upper bounds, so what separates a small budget from a weak fitter is the detail below the program's own finest landform radius of 12 world units, which no fit can place and whose p95 is 2.428 full and 1.909 shore — under both thresholds. ADR-0054 records this without growing the budget or moving the threshold: 101 two-dimensional landforms is around 600 numbers for a 300-unit-radius island, which is the sampled elevation field the budget exists to prevent. The shore gate has a separate cause worth keeping in view — fine scales are refused beyond normalised coastal radius 0.8 to protect the coastline gate, which leaves the shore band with nothing but a three-number shelf.
- One authored asset name can hide two different forms, and a per-kind average hides which. `bamboo_forest_fbx` is placed 130 times as two clusters with nothing between them: 72 standing clumps of 19,908 triangles at a height-to-width aspect near 3.9, and 58 flat pieces near 0.065 — 56 of 80 triangles and two mats of 1,748. Because Target AABB Extent is a hard output target, generating a standing clump for a piece 0.7 units tall produced a squashed clump, and the mixed average of 17.78 made `bamboo` the largest single contributor to the failing surface gate at 3.44 of 9.45, more than the sixteen distant mountains together. `resolveFamily` now splits them on measured proportion, the way it already splits `paving-slab` into `deck` on measured footprint, and the 58 beds fell to 2.13 against a threshold of 2.3479.
- Surface sampling allocates its budget per mesh, which penalises the semantic part structure the milestone requires. `sampleEntitySurface` gives each mesh in an entity `min(96, max(12, sqrt(triangles) * 3))` triangle-uniform samples: sub-linear and floored, so a small part is sampled far more densely than its share of the entity. An authored placement is one mesh whose thin culms are about three parts in a thousand of its triangles and are therefore never sampled; a generated clump carrying culms and foliage as separate parts gives its culms a floor of twelve samples, each measuring its distance to a canopy tens of units away. Measured across every kind, `bamboo` reads 4.12 reference-to-candidate against 39.32 candidate-to-reference while palm reads 0.73 and blossom 1.18, so the candidate's canopy already covers the authored one and the whole penalty is one-directional. Matching the authored triangle allocation is still the faithful response and cut generated triangles from 664,649 to 529,390, but the metric's own allocation is what bounds the result.
- Every Material Family's albedo was hand-written, and the authored materials cannot correct one directly: measured from the assembled scene, all 236 palm placements and 442 of the vegetation materials carry a white base colour with a texture, so the colour lives in maps the Production Runtime may not load. That is the same fact ADR-0051 met when `desaturate-albedo` measured DeltaE 0.057 and had to be replaced. `tools/development/measure-material-albedo.mjs` measures each family's area-weighted mean linear reflectance from those maps and from base colours where a surface has none, alpha-weighted so a leaf card's transparent field does not dilute the leaf. Carrying the result took global appearance DeltaE from 13.168 to 12.504, `palm-foliage` from 54.84 to 38.87, and `distant-rock` to agreement at [179 177 163] against a reference [179 176 161].
- The residual left after a family's material is measured belongs to geometry, and fitting it would buy the appearance layer with the geometry layers. `palm-foliage` renders 2.9 times too bright with a measured albedo, because a dense authored canopy shadows itself and a sparser candidate cannot. A rendered fitting loop wants an albedo darker than the authored material actually is, so `tools/development/measure-appearance-direction.mjs` reports the direction of each family's residual and deliberately never writes one. Before it, the pass recorded only the magnitude of a family's error and not its direction, which is why a per-family DeltaE could not be acted on at all.
- An authored family lives on the placement group, not on the mesh, and two resolutions of it will disagree. A palm frond is a mesh named `Mesh_79036` under a `PalmTree__palmtree_5__0001` group and a wildlife rig's meshes are unnamed, so keying a family off the mesh's own name reached 38 per cent of `palm-foliage` and 13 per cent of `creature-fur` while reporting full confidence in both. `scene-placements.mjs` now exports `resolveRenderableSemantics` so the recipe and every development tool share one rule.
- The surface sample budget belongs to an entity, not to each of its meshes (ADR-0055). The per-mesh formula is sub-linear and floored, so a small part drew far more points than its share of the geometry and Scene Surface Parity depended on how a form was divided into parts — penalising exactly the semantic part structure a generator is required to have. A generated bamboo clump gave its culms 28 per cent of its samples for 13 per cent of its triangles while the authored placement is one mesh whose culms are three parts in a thousand and were never sampled, reading 4.12 one way against 39.32 the other. The reference measurement also kept its own copy of the rule and applied it per renderable, so an authored placement of five meshes drew five budgets. One shared implementation, grouped by placement, took `surface p95` from 9.3025 to 7.2469 and `bamboo` from 29.01 to 7.34; `worst entity surface p95` went the other way, 143.10 to 155.20, which is what a corrected measurement does.
- A ratchet whose baseline was measured by a different rule is not a ratchet. Palm's recorded surface limit was re-baselined from 6.5 to 6.8 when ADR-0055 changed the sampler, and the two facts that make that safe are recorded beside it: palm's generated geometry is byte-identical, and the same correction improved the aggregate and `bamboo` by far more than it cost palm. The world-geometry thresholds moved for the same reason and by one to five per cent, and the guard that used to forbid any movement now requires each move to be declared with both values and to start from the value the frozen table records — nothing moves silently, and nothing may claim a move it did not make.
- Semantic part structure is a program, not a table (ADR-0056). Fourteen entities were missing parts and all fourteen were pandas: the authored rig is 15 meshes and the generator emitted 6, or 7 when a coin flip said so. A part count does not say what to build, so the topology was measured first — every rig is 3 body masses chained along its own long axis plus 4 legs of 3 stacked segments mirrored across it, and the two distinct panda assets agree on the proportions to within a few per cent once each is normalised by its own Target AABB Extent. What crossed into production is ~20 fractions and a topology, independent of triangle count and of how many pandas the island has. Deficit 9 to 0, entities missing parts 14 to 0, and the panda's own surface p95 improved 1.2691 to 1.1581 as well.
- A rig's facing and its authored AABB are two different frames, and mixing them is silent. The wildlife rigs face their own local -Z: recovered by un-rotating each world offset by the entity's Typed Scene Orientation yaw, all fourteen put the head mass at negative Z. Building the head at +Z satisfies the Target AABB Extent contract exactly and seats every panda's head where its tail is, so a test asserts the chain's order rather than trusting a comment. For the same reason the proportions may only be measured from yaw-0 representatives: an authored AABB is a world AABB, so for a yawed placement it bounds the rotated form and its axes are not the rig's own.
- Structure that is measured rather than invented tends to pay twice. `panda-statue` is one merged 204-triangle authored mesh with no component deficit to fix, and putting it on the same measured quadruped program improved its surface p95 from 1.8857 to 1.7037 — so it keeps no special case. The corollary is the reason the component *delta* stays ungated at 14: nearly every authored object on this island is a single merged mesh, an export artefact rather than a claim about parts, so gating the delta would pay a generator to merge its parts away.
- Normalise a surface residual by the entity's own diagonal before believing a ranking. Absolute `surface p95` per kind ranks mountain 103, plaza 42.7, pavilion-single 21.1 — which reads as a list of broken kinds. Divided by each kind's own diagonal the whole scene sits between 10 and 33 per cent, and mountain (9.6%) and bamboo (10.9%) are the *best* while `stone-block` (33.1%) and `dumpling-house` (29.9%) are the worst. The absolute ranking is mostly a ranking of object size, and the uniform fifth-of-a-diagonal residual is the signature of right-bounding-box, wrong-form. It also says what a village fix can be expected to buy: weighted by entity count, the whole village contributes about 10 per cent of the aggregate `surface p95` against mountain's 34 and palm's 21, so village work has to be justified on the fixed-camera layer and human review rather than on the world-geometry gate.
- Matching a coverage fraction without matching where the coverage is makes the silhouette worse while making the pixel ratio look fixed. A candidate that fills its Target AABB Extent against a reference plate covering fraction c scores IoU = c exactly — which is why `plazas` reads 0.367 against authored plates covering 0.23 to 0.34 of their boxes. A candidate that covers c in uncorrelated places scores c²/(2c − c²), about 0.21 at c = 1/3. So thinning the plaza to the right area is a regression that reports as progress on the pixel ratio, and shape has to come with it. The same reasoning inverts where the reference is *known empty*: the authored flat-span bridge has 95 per cent of its geometry in the top two height deciles, so removing candidate mass from the bottom seven shrinks the union without touching the intersection and can only help.
- The Target AABB Extent contract means a candidate cannot cover less of its bounding rectangle by shrinking — only by being concave or perforated. `placeEntity` scales the local form so its generated AABB equals the declared extent exactly, so any inset plate is scaled straight back out to the walls. The authored plates reach the walls too and still cover a third of the rectangle, and the offline reach measurement says how: mean |offset| x 2 as a fraction of extent is 0.31 to 0.44 for the four decks against 0.5 for a uniformly filled rectangle, so the authored deck is centre-heavy with narrow arms out to the box edges rather than a small centred pad. Every authored plate is centred to within 0.03 to 0.08 of its box centre, which also means a centred candidate is correlated with it and does not pay the uncorrelated IoU penalty.
- Reach and centroid, measured from the same samples on both subjects, say which direction each kind is wrong, and the directions differ. Footprint reach: decks authored 0.31-0.44 against candidate 0.54-0.58 (candidate too spread out), pavilion authored 0.385/0.311 against candidate 0.272/0.289 and pavilion-single authored 0.481/0.338 against candidate 0.318/0.282 (candidate too narrow), bridge authored 0.39-0.51 against candidate 0.56/0.52, name-plate authored 0.42/0.37 against candidate 0.51/0.61. A single global correction would therefore make two of these worse, which is why the village is per-kind work and not one massing tweak.
- A group that over-draws hides its neighbours' errors, so per-group rendered scores are not independent. Shrinking the plaza from filling its box to its measured footprint left `paths`, `rocks`, `geography` and `bridges` untouched in code and moved all four: `paths` contour 111.5 to 81.1 and `geography` 16.7 to 12.4 improved, while `rocks` contour nearly doubled from 25.6 to 47.9 and `bridges` IoU slipped 0.370 to 0.360. The plaza had been covering them. Re-read the per-group ranking after every over-draw is fixed, and treat a group's score as provisional while any neighbour draws more pixels than the reference.
- Coverage is a union, and the first version of any two-component footprint program forgets that. The plate's crossing paths and perimeter walk overlap at four places; inverting each component's area independently built 0.527 against a target of 0.568. A short fixed-point loop that grows both areas by the current overlap converges in about three passes. Its own test caught this, which is the argument for writing the coverage assertion before believing the closed form.
- Check whether the accepted representation is actually being used before treating a group as a fitting problem. `rocks` failed on shape for the whole milestone while the repository already had the Bounded Support-plane Polyhedron proven under `stone-geometry-baseline-v2`; the scene generator was building a jittered sphere. Measured along the same 24 canonical directions the candidate sat inside the authored form in 23 of 24 of them, because a sphere inscribed in a box touches the six face centres and falls short everywhere else. Switching to the accepted representation halved the group's contour p95, 47.93 to 24.06.
- A support-plane polyhedron can be evaluated radially instead of by hull construction: the boundary along a ray is `min over i of support[i] / (ray dot direction[i])` over the directions the ray points into, so a ring-and-side lattice reproduces the silhouette in a few lines. That matters because `objects/stone-generator.js` is hash-frozen under `stone-geometry-baseline-v2` and cannot be edited to export its hull code. Import the direction set from it rather than restating the 24 vectors, and declare the module in both `PERMITTED_RUNTIME_PATHS` and `PRODUCTION_FILES` — the coupling to another milestone's frozen artefact is real and is better made visible than avoided by a second copy.
- Do not assert a family statistic against one sample of the family, and do not compare two subjects measured by different methods. The rock profile test first compared one seed's vertices against a mean pooled over 39 authored placements, and failed on a direction whose authored spread is 0.168 — it was measuring the family's own variation. Pooling 39 generated seeds still failed, because generated vertices reach the AABB walls exactly while a 96-sample authored profile cannot. The test that holds reads the measurement tool's own output, where both subjects go through the same sampler, and asserts the mean absolute profile error (0.0428) plus the systematic-shortfall count rather than a worst-direction bound invented to fit.
- A measured control transferring between kinds does not mean the form family transfers. The plaza's `footprintCoverage` and `perimeterShare` applied cleanly to bridges, and a third control fell out of evidence already on disk — the two bridges agree on their vertical profile spread (0.1842, 0.1871) and differ two-fold on where the band sits, so the spread is a family constant and only `deckHeight` is per-entity. Built as a plate at that height, the group's pixel ratio went 2.22 to 1.58 and contour, depth and normals all improved, while its silhouette IoU fell 0.362 to 0.295. Reverted. The rectangle decomposition had already said why and the attempt did not read it: the two largest rectangles of each authored bridge lie along axes at -119.2 and +136.1 degrees, so the two bridges run along opposite diagonals of their own boxes and a centred cross-and-walk plate is neither. A bridge deck is a band along an axis, and the axis is a fourth per-entity control.
- Read the decomposition you already paid for before designing the form. The plate-footprint measurement records up to six rectangles per authored asset, and their positions carry the form's axis, not just its coverage. Both village attempts that were reverted this milestone — the architecture massing fit and the bridge plate — failed on something the existing evidence had already recorded and the design step skipped.
- Check whether a blocker is yours before recording it as the host's. Ticket 15 sat as "Firefox and Safari are not installed" for the whole milestone while the actual obstacle was `smoke-local-scene.mjs` passing `--use-angle=swiftshader --enable-unsafe-swiftshader` on every launch. Every run in the milestone is software-rasterised and the frozen reference observation says so in its own `hostKey`, so a gate that rejects software rendering could not have passed on any number of browsers. The host turned out to have an Intel Arc 140T reachable with `--use-angle=default` (max texture 16384 against SwiftShader's 8192).
- Ask for hardware acceleration per run, never harness-wide. `browserArguments` are appended after the harness's own flags and later Chromium flags win, so one gate can request the real adapter while every other gate keeps the SwiftShader determinism its frozen thresholds were calibrated through. Switching the shared harness would have moved every rendered measurement in the milestone at once while looking like a one-line improvement. And a GPU host is a *different host* in the cross-host sense, not a better measurement of the same one, so its numbers belong in the `describeNormativeHost` record and nowhere near a threshold.
- Firefox and Safari are not an install problem. Gecko speaks WebDriver BiDi and WebKit speaks the WebKit inspector protocol, so the shared CDP harness cannot drive either even where they are present; building a BiDi transport is the work. Chrome is not engine diversity either — same Chromium, same ANGLE, same adapter as Edge.
- A development tool that silently matches nothing is worse than one that throws. `measure-architecture-massing.mjs --kinds lantern` printed an empty report for a whole round because the tool hardcoded three groups and skipped anything else; and it suppressed the *pair* of reach values whenever either subject had no samples in a height band, which hid the authored value in exactly the bands where the candidate has no geometry. Both are now fixed — named kinds search every group and an unknown kind throws with the list, and each side prints its own reach.
- One revolved surface beats a stack of primitives when the authored body is smooth. The reverted architecture attempt matched its massing profile and still raised group contour distance a fifth, because eight boxes standing in for one authored mesh add silhouette edges and depth discontinuities the reference does not have. Rebuilding `lantern` and `npc-statue` as a lathe through their measured half-extent profile improved IoU, contour, normals and depth at once and *cut* 44 draw calls. A radially averaged reach profile is also exactly what a revolved surface describes — giving it a plan shape the measurement cannot see would be inventing.
- Rank a group by its own authored pixels before assuming what it contains. `structures` reads as a village of shops and its biggest slice is two *tree* kinds: `wish-tree` 3,944 authored overview pixels over 3 placements and `swing-tree` 1,001 over 5, together 40 per cent of the group and more than every shop and booth combined. Both are missing their ground structure rather than their canopy — authored base reach 0.616 to 0.661 against a candidate 0.095 to 0.314 — because the placement is a tree *plus its structure* (a swing frame, a wish rack) and its AABB covers the whole assembly while `broadleaf` puts a bare trunk on the axis.
- Reach is a mean, so matching it with solid geometry over-fills. A thin frame at radius 0.6 reads reach 0.6 while carrying almost no area; widening a trunk into a solid taper to hit that number would draw about seven times the silhouette width down low. Match the share *and* the reach together: mass belongs at the measured radius, not on the axis. And a radially averaged profile cannot say how many uprights produced it — declare the count, say it is declared, and check the built pair rather than presenting a chosen number as measured.
